...
/**
 * Rule action that discounts all products in a given category by a given percentage. The NUM_ITEMS_KEY specifies the maximum number of items to
 * apply the discount to. If this value is 0, then the action will discount an unlimited number of items.
 */
@Entity
@DiscriminatorValue("cartCategoryPercentDiscountAction")
@DataCache(enabled = false)
public class CartCategoryPercentDiscountActionImpl extends AbstractRuleActionImpl implements RuleAction {
	/**
	 * Serial version id.
	 */
	...
	private static final RuleElementType RULE_ELEMENT_TYPE = RuleElementType.CART_CATEGORY_PERCENT_DISCOUNT_ACTION;

	/** Set of keys required for this rule action. */
	protected static final String[] PARAMETER_KEYS = new String[] { RuleParameter.DISCOUNT_PERCENT_KEY, RuleParameter.NUM_ITEMS_KEY,
	...
	public String getRuleCode() throws EpDomainException {
		...
		sbf.append("\n\t insert(new CartCategoryPercentDiscountImpl(\"").append(RULE_ELEMENT_TYPE).append("\", ");
