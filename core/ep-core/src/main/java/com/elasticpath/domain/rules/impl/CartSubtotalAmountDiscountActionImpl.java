 ...
 */
@Entity
@DiscriminatorValue("cartSubtotalAmountDiscountAction")
@DataCache(enabled = false)
public class CartSubtotalAmountDiscountActionImpl extends AbstractRuleActionImpl implements RuleAction {
	/**
	 * Serial version id.
	 */
	private static final long serialVersionUID = 5000000001L;

	private static final RuleElementType RULE_ELEMENT_TYPE = RuleElementType.CART_SUBTOTAL_AMOUNT_DISCOUNT_ACTION;

	private static final String[] PARAMETER_KEYS = new String[] { RuleParameter.DISCOUNT_AMOUNT_KEY };
	 ...
	 * Constructor.
	 */
	public CartSubtotalAmountDiscountActionImpl() {
	...
	public RuleExceptionType[] getAllowedExceptions() {
		return null;
		// return (String[]) ALLOWED_EXCEPTIONS.clone();
	}
	...
	public String getRuleCode() throws EpDomainException {
		...
		sbf.append("\n\t insert(new CartSubtotalAmountDiscountImpl(\"").append(RULE_ELEMENT_TYPE).append("\", ");
	...
	public void validate() {
		...
		if (amount.compareTo(BigDecimal.ZERO) <= 0) {
			throw new EpDomainException("Invalid percent parameter: " + amount + ". Amount parameter must be great than 0.");
		}
	}
