...
/**
 * Rule exception that excludes a product to be qualified for a rule condition or a rule action. Requires parameter with key
 * RuleParameter.PRODUCT_ID_KEY to specify the product id.
 */
@Entity
@DiscriminatorValue("productException")
@DataCache(enabled = false)
public class ProductExceptionImpl extends AbstractRuleExceptionImpl implements RuleException {
	/**
	 * Serial version id.
	 */
	private static final long serialVersionUID = 5000000001L;

	private static final RuleExceptionType EXCEPTION_TYPE = RuleExceptionType.PRODUCT_EXCEPTION;

	private static final String[] PARAMETER_KEYS = new String[] { RuleParameter.PRODUCT_CODE_KEY };
