 ...
 */
@Entity
@DiscriminatorValue("productInCartCondition")
@DataCache(enabled = false)
public class ProductInCartConditionImpl extends AbstractRuleElementImpl implements RuleCondition {
	/**
	 * Serial version id.
	 */
	private static final long serialVersionUID = 5000000001L;

	private static final RuleElementType RULE_ELEMENT_TYPE = RuleElementType.PRODUCT_IN_CART_CONDITION;
	...
	public String getRuleCode() throws EpDomainException {
		...
		sbf.append(" delegate.cartContainsProduct(cart, \"");
		sbf.append(getParamValue(RuleParameter.PRODUCT_CODE_KEY));
		sbf.append("\", \"").append(getParamValue(RuleParameter.NUM_ITEMS_QUANTIFIER_KEY));
		sbf.append("\",").append(getParamValue(RuleParameter.NUM_ITEMS_KEY));
