 ...
 */
@Entity
@DiscriminatorValue("cartProductPercentDiscountAction")
@DataCache(enabled = false)
public class CartProductPercentDiscountActionImpl extends AbstractRuleActionImpl implements RuleAction {
	/**
	 * Serial version id.
	 */
	...
	private static final RuleElementType RULE_ELEMENT_TYPE = RuleElementType.CART_PRODUCT_PERCENT_DISCOUNT_ACTION;
	...
	public String getRuleCode() throws EpDomainException {
		...
		sbf.append("\n\t insert(new CartProductPercentDiscountImpl(\"").append(RULE_ELEMENT_TYPE).append("\", ");
		sbf.append(this.getRuleId()).append("L, ");
		sbf.append(this.getUidPk()).append("L, \"");
		sbf.append(this.getParamValue(RuleParameter.PRODUCT_CODE_KEY)).append("\", \"");
		sbf.append(this.getParamValue(RuleParameter.DISCOUNT_PERCENT_KEY)).append("\", \"");
		sbf.append(this.getExceptionStr());
